...
if ( $opts->{build} ) {
    ...
    foreach my $file ( sort { $a cmp $b } @files ) {
        my @losetup = ("losetup", "--show", $opts->{readonly} ? "-fr" : "-f", $file);
        print STDERR "RUNNING: @losetup\n";
    ...
    print "Base Device Table: ($opts->{name})\n$table\n";
    my @dmsetup = ("dmsetup", $opts->{readonly} ? ( "-r", "create" ) : ("create"), $opts->{name});
    print STDERR "RUNNING: @dmsetup\n";
    ...
    foreach my $partition (@partitions) {
        ...
        printf "0 %d linear /dev/mapper/%s %d (%s%d)\n",
          $partition->{length},
