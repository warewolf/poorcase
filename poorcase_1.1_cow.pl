...
GetOptions( $opts, "build", "destroy", "options=s", "name=s", "cow=s", "help|?" )
...
pod2usage(
    ...
    -verbose => 1,
    -exitval => 1
) unless ( $opts->{options} eq "ro" or $opts->{options} eq "rw" or $opts->{options} eq "cow");

if ( $opts->{options} eq "ro" or $opts->{options} eq "cow") {
    $opts->{readonly} = 1;
} elsif ( $opts->{options} eq "rw" ) {
    $opts->{readonly} = 0;
} else {
    pod2usage(
        -msg     => "Dazed and confused by --option $opts->{option}.  Use --option rw or --option ro or --option cow",
        -verbose => 1,
        -exitval => 1
    );
}
...
if ( $opts->{build} ) {
    ...
    if ($opts->{options} eq "cow") {
       my $file = abs_path($opts->{cow});
       open( LOSETUP, "-|", "losetup", "--show",
            "-f", $file )
          or die "Couldn't run losetup ($!)";
        my $cowdev = <LOSETUP>;
	...
	my $device = "/dev/mapper/" . $opts->{name};
        my $size = `blockdev --getsize $device`;
        chomp $size;
        $table = "0 $size snapshot $device $cowdev p 8";
        printf STDERR " COW: $table\n";
        $opts->{name} .= "-cow";
        open( DMSETUP, "|-", "dmsetup", "create",
            $opts->{name} )
          or die "Couldn't create device-mapper device ($!)";
        print DMSETUP $table;
        close DMSETUP;
    ...
    foreach my $partition (@partitions) {
        open( DMSETUP, "|-", "dmsetup", $opts->{readonly} ? ( "-r", "create" ) : ("create"),
